// Modified by 28add11 to add file dumping for testing a chip
// Each newline is a sample or 
	...
	for (int sample_index = 0; sample_index < samples; sample_index += QOA_SLICE_LEN) {
		for (int c = 0; c < channels; c++) {
			...
			for (int si = slice_start; si < slice_end; si += channels) {
				...
				// First we put sf_quant, then qr, then the reconstructed sample
				fprintf(debugF, "%i %i %i\n", scalefactor, quantized, reconstructed);
